...
public class JFRSequenceIterator implements MultiDataSetIterator {
    ...
    public JFRSequenceIterator(int seed, int batchSize, int totalBatches, int numdigits, int timestep) {
    ...
    public MultiDataSet next(int sampleSize) {
        ...
        INDArray encoderSeq = Nd4j.zeros(sampleSize);
        INDArray decoderSeq = Nd4j.zeros(sampleSize);
        INDArray outputSeq = Nd4j.zeros(sampleSize );
